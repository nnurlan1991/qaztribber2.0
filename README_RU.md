...
QazTriber — нативное десктопное приложение на базе Tauri. Аудио обрабатывается локально: Python sidecar (FastAPI + ИИ модель для распознавания) работает в фоне, а нативное окно Tauri показывает интерфейс. Браузер не нужен.

Доступны ровно две модели:

| В интерфейсе | ИИ модель | Для чего |
| --- | --- | --- |
...
> Меньшая модель имеет 220M параметров, не 200M.
...
- `backend/` — FastAPI, очередь задач, подготовка аудио и ИИ модель для распознавания.
